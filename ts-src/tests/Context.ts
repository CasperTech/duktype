import 'mocha';
import { Context, ObjectScope, TypescriptCompiler } from '../../index';
...
describe('ObjectScope', () =>
{
    ...
    it('objects should be garbage collected properly', (done: Done) =>
    {
        ...
        // Should be the global object only
        if (objectRefs === 1)
        {
            done();
        }
...
describe('Complex operations', () =>
{
    ...
    it('should handle many simultaneous eval calls', async () =>
    {
        ...
        globalObj.setProperty('test1', () =>
        {
            return 'bacon';
        });
        globalObj.setProperty('test2', () =>
        {
            return ' and ';
        });
        globalObj.setProperty('test3', () =>
        {
            return 'chips';
        });
        ...
        async function run()
        {
            ...
            return ctx.eval('test1() + test2() + test3();');
        }
...
describe('TypescriptCompiler', () => {
    it('should compile and run the example typescript code', async() =>
    {
        if (ctx === undefined)
        {
            throw new Error('Context is undefined');
        ...
        const obj = await globalObj.createObject('console');
        await obj.setProperty('warn', (message: string) => {
            // Suppress the unhandled promise rejection message
        })

        const compiler = new TypescriptCompiler();
        const source = fs.readFileSync(path.resolve(__dirname + '/../../compileMe.ts.example')).toString('utf-8');
        const compiledCode = await compiler.compile(source);
        try
        {
            await ctx.eval(compiledCode);
        }
        catch(err)
        {
            if (err.message && err.message === 'Successful failure')
            {
                // Expected result
                return;
            }
        }
        throw new Error('This code should have produced and rejected a promise');
    });
});
